...
`src/main/db/ssh-tunnel.ts` forwards a local port over a bastion so Postgres and MySQL can reach a host that is not publicly routable. D1 is excluded by `isSshEnabled()` - it speaks REST, so there is no socket to forward.

**The transport is a local listener, not a driver stream hook.** Each tunnel binds a `net.Server` to `127.0.0.1:0` and pipes every inbound socket through `client.forwardOut()`; the driver then dials that port and is otherwise untouched. `pg` and `mysql2` both expose a `stream` option that would skip the listener, but their signatures differ and TLS-over-custom-stream is the fragile path - a loopback forward behaves identically for both. Port `0` means two tunnels can never collide, and binding `127.0.0.1` keeps the forwarded port off the network.
...
The SSH secrets are **flat top-level fields** (`sshPassword`, `sshPrivateKey`, `sshPassphrase`), matching how the D1 credentials sit rather than a nested block, because `connections-store`'s `SENSITIVE_FIELDS` is a list of top-level keys. Adding three entries there gives SSH the whole existing machinery - sealing, the plaintext migration, and the rule that a secret which fails to unseal is kept on disk untouched. A nested block would have meant rewriting that walker for nothing. A private key is stored as **contents**, not a path: the store encrypts it like every other secret, and a path silently breaks when the file moves. `connections:pick-ssh-key` is the file dialog that reads it.

Tunnel and pool have the same lifetime, so `disconnectPool` closes both - including when there was no pool, since a failed first connect can leave one standing. If the bastion drops under a live pool, the listener would stay up and hand out sockets it can no longer forward, so `client.on('close')` tears down the listener _and_ evicts the pool through `addTunnelPoolEvictor` (each driver registers its own; evicting an id it does not hold is a no-op).

**`disconnectPool` awaits the in-flight connect before closing anything.** During a cold connect neither the pool nor the tunnel is registered yet - both live in their `pending` maps - so an SSH handshake (seconds) is long enough for Disconnect/Edit/Delete to close nothing and leave both standing once it resolved.
...
`tests/main/db/ssh-tunnel.test.ts` unit-tests `buildSshConnectConfig` (where every decision that can be wrong lives) and then stands up a real `ssh2.Server` plus a TCP echo server on loopback to prove bytes actually round-trip - the piping is not something a unit test can reach. Note the `client.on('error')` on that test server: rejecting a host key aborts the key exchange, and unhandled it fails the run from outside any test.
